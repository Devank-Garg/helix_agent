"""helix_agent.api.anthropic — Anthropic-native API client with retry logic and streaming."""
...
class AnthropicClient(BaseClient):
    ...
    async def stream_message(self, request: MessageRequest) -> AsyncIterator[StreamEvent]:  # type: ignore[override]
        """Open a streaming SSE connection, yielding :class:`StreamEvent` objects.
        ...
        stream = MessageStream(response=response, request_id=request_id)
        async for event in stream:
            yield event
